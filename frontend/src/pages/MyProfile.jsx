...
const MyProfile = () => {
  ...
  return userData && (
    <div className='max-w-lg flex flex-col gap-2 text-sm '>
        ...
        isEdit
          ? <label htmlFor='image'>
            <div className='inline-block relative cursor-pointer'>
              <img className='w-36 rounded opacity-65' src={image ? URL.createObjectURL(image) : userData.image} alt="err" />
              <img className='w-10 absolute bottom-12 right-12' src={image ? 'null' : assets.upload_icon} alt="" />
            </div>
            <input onChange={(e) => setImage(e.target.files[0])} type="file" id='image' hidden />
          </label>
          :
          <img className='w-36 rounded' src={userData.image} alt="err" />
      }
      ...
      <div>
        <p className='text-neutral-500 underline mt-3'>BASIC INFORAMTION</p>
